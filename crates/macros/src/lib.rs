use meta_utils::{find_arg, find_constructor};
...
pub fn jsonize_singleton(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let constructor_name = find_constructor(attr_args.clone())
        .expect("Missing `constructor` argument for `jsonize_singleton`");
    let params_url = find_arg(attr_args.clone(), "params_url")
        .expect("Missing `params_url` argument for `jsonize_singleton`");
    let pk_url =
        find_arg(attr_args, "pk_url").expect("Missing `pk_url` argument for `jsonize_singleton");
    ...
    let params_quote = quote! { include_bytes!(#params_url) };
    let pk_quote = quote! { include_bytes!(#pk_url) };
    let instance_code = quote! {
        ::once_cell::sync::Lazy::new(|| {
            #self_ty::#constructor_ident( #params_quote, #pk_quote )
        })
    };
