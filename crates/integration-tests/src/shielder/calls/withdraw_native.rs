...
pub fn prepare_call(
    ...
    let mut calldata = shielder_account.prepare_call::<WithdrawCallType>(
        &params,
        &pk,
...
mod tests {
    ...
    fn handles_withdraw_transfer_failure(mut deployment: Deployment, #[case] token: TestToken) {
        ...
        let (calldata, _) = prepare_call(
            ...
            token,
            PrepareCallArgs {
                // Should trigger a revert of both the native call (thanks to `REVERTING_BYTECODE`)
                // and of the ERC20 call (thanks to the `FakeERC20Token` implementation).
        ...
        match token {
            ...
            TestToken::FakeERC20 => {
                assert_matches!(result, Err(ShielderContractErrors::ERC20TransferFailed(_)))
            }
        };
        assert!(actor_balance_decreased_by(
            &deployment,
