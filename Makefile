...
ifeq ($(NETWORK),anvil)
	...
	PRIVATE_KEY=$(PRIVATE_KEY) OWNER_ADDRESS=$(shell cast wallet address $(PRIVATE_KEY)) forge script DeployShielderScript --broadcast --rpc-url anvil --sender $(shell cast wallet address $(PRIVATE_KEY))
else
